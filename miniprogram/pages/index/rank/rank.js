// miniprogram/pages/index/rank/rank.js
...
Page({
    ...
    db.collection('questionMenu').get().then(res => {
      ...
      if (res.data.length > 0) {
        for (var i = 0; i < res.data.length; i++) {
          questionMenu.push(res.data[i].name);//questionMenu只是存了各套题的名字
        }
      }
      this.setData({
  ...
  changeMenu(e) {
    ...
    console.log()
    wx.navigateTo({
      url: '../rankList/rankList?id='+this.data.objectQuestionMenu[this.data.index].id 
    })
  }
})